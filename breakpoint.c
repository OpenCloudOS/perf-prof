...
static const char *breakpoint_desc[] = PROFILER_DESC("breakpoint",
    ...
    "",
    "SYNOPSIS",
    "    Track read, write, or execute accesses to specified addresses using",
    "    PERF_TYPE_BREAKPOINT PMU. Configure PERF_SAMPLE_REGS_INTR to capture",
    "    general-purpose registers at breakpoint trigger time for instruction",
    "    analysis.",
    "",
    "    Each process has a maximum of 4 hardware breakpoints.",
    "",
    "    The -g option enables call stack sampling to identify the code path",
    "    triggering the breakpoint.",
    "",
    "WRITE VALUE TRACKING (x86 kernel addresses only)",
    "    For write breakpoints on x86 kernel addresses, the tool decodes write",
    "    instructions to extract the written value from the source operand,",
    "    combined with sampled registers. This enables tracking value changes",
    "    over time without reading memory directly (which would race with writers).",
    "",
    "    Supported instructions include MOV, CMPXCHG, ADD, SUB, OR, AND, XOR,",
    "    INC, DEC, NOT, NEG, shift/rotate (SHL/SHR/SAL/SAR/ROL/ROR), and bit",
    "    operations (BTS/BTR/BTC). Note: XCHG, XADD are not yet supported.",
    "",
    "    This is useful for analyzing spinlock acquire/release paths, reference",
    "    counting changes, and other atomic operations in the kernel.",
    "",
    "FILTER OPTION",
    "    The --filter option filters samples based on the written value,",
    "    using 'data' as the global variable referencing the value.",
    "    E.g., --filter 'data > 0' prints only when written value > 0.",
    "",
    "EXAMPLES",
