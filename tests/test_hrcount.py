...
def test_sched_switch_wakeup_cpus(runtime, memleak_check):
    #perf-prof hrcount -e sched:sched_switch,sched:sched_wakeup//cpus=0-2/ --period 50ms -i 1000 --perins
    prof = PerfProf(["hrcount", '-e', 'sched:sched_switch,sched:sched_wakeup//cpus=0-2/', '--period', '50ms','-i', '1000', '--perins'])
    for std, line in prof.run(runtime, memleak_check):
        result_check(std, line, runtime, memleak_check)


def test_sched_switch_wakeup_cpu0(runtime, memleak_check):
    #perf-prof hrcount -e sched:sched_switch//cpus=0/,sched:sched_wakeup/target_cpu==0/ --period 50ms -i 1000
    prof = PerfProf(["hrcount", '-e', 'sched:sched_switch//cpus=0/,sched:sched_wakeup/target_cpu==0/', '--period', '50ms','-i', '1000'])
    for std, line in prof.run(runtime, memleak_check):
        result_check(std, line, runtime, memleak_check)