...
- **默认注入 `--order`**：若透传参数里没有 `--order`，脚本会自动补上。原因是
  uprobe/kprobe 事件按 CPU 分散写入独立 ringbuffer，同一次函数调用的 entry
  和 return 完全可能落在不同 CPU 上；跨 CPU ringbuffer 不做严格时间排序时，
  return 事件可能"早于"entry 事件被回调处理，导致栈匹配错乱、耗时为负。
  默认打开 `--order` 才能保证 `_entry` / `_return` 拿到的 `event._time` 是
  全局有序的。若用户显式传入 `--order`，脚本不重复添加。
...
树形，用 `|- / |  |-` 缩进呈现调用嵌套；内核函数分支符改用 `|→`（跟用户
态 `|-` 视觉区分，同时保持宽度 1 不破坏列对齐）。每个 dump 窗口在表头前
先输出一行 wall-clock 时间戳（精确到秒），方便把统计对齐到真实事件、
日志、监控系统时间轴。
